public class UserTableShardingTest {
    public static void main(String[] args) {
        String s="CREATE TABLE `t_user_%d`  (\n" +
                ...
                "  `delete_time` bigint(0) NULL DEFAULT NULL COMMENT '注销时间戳',\n" +
                "  `create_time` datetime(0) NULL DEFAULT NULL COMMENT '创建时间',\n" +
                "  `update_time` datetime(0) NULL DEFAULT NULL COMMENT '修改时间',\n" +
                "  `del_flag` tinyint(1) NULL DEFAULT NULL COMMENT '删除标识 0：未删除 1：已删除',\n" +
                "  PRIMARY KEY (`id`) USING BTREE,\n" +
                "  UNIQUE INDEX `username`(`username`) USING BTREE\n" +
                ") ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci ROW_FORMAT = Dynamic TABLESPACE = `innodb_system`;";
        for (int i = 0; i < 16; i++) {
